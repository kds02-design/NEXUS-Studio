...
import {
  ...
  Layers, Send, Plus, Clock, Trash2, ClipboardCheck, Maximize2, StretchVertical,
} from 'lucide-react';
...
// 히스토리/리스트 이미지로 안전하게 표시 가능한 src 인지 — blob: URL 은 다른 세션에서 무효라 제외.
// data: / http(s) / firebase storage URL 만 허용. (구버전이 blob: 저장한 항목 호환용 guard.)
const isUsableImageSrc = (src) => typeof src === 'string' && (src.startsWith('data:') || src.startsWith('http'));
...
export default function DesignEvaluator() {
  ...
  // 이미지 표시 모드 — 'width' 폭 맞춤(긴 portrait 는 세로 스크롤), 'screen' 화면 전체 맞춤(브랜드웹 모바일·세로 배너 등).
  // aspectRatio 기반으로 새 이미지 로드 시 자동 결정. 사용자가 토글하면 그 이미지에서는 override 유지.
  const [fitMode, setFitMode] = useState('width');
  const fitModeOverrideRef = useRef(false);
  ...
  // fitMode 자동 결정 — 사용자 override 가 없을 때만. 폰 비율(9:14.5~9:22) 이면 화면 맞춤, 아니면 폭 맞춤.
  // 새 이미지 업로드 / localStorage 복원 둘 다 커버.
  useEffect(() => {
    if (!previewUrl || !aspectRatio || fitModeOverrideRef.current) return;
    setFitMode(aspectRatio >= 0.40 && aspectRatio <= 0.65 ? 'screen' : 'width');
  ...
  const saveEvaluationToHistory = async (result, image, category) => {
    ...
    // 반드시 dataURL 로 변환한 뒤 압축한다.
    let normalized = image;
    if (typeof image === 'string' && image.startsWith('blob:')) {
      try {
        normalized = await blobUrlToBase64(image);
      } catch (e) {
  ...
  const loadHistoryItem = (item) => {
    // 구버전이 blob: URL 을 저장한 항목은 다른 세션에서 무효 → null 로 다뤄 빈 뷰 표시.
    setPreviewUrl(isUsableImageSrc(item.image) ? item.image : null);
    setResultData({
      title: item.title,
  ...
  // banner / promotion / brandweb / brandwebSub / typo2d / typoRender / typoMotion 활성 버전을 한꺼번에 로드 → 카테고리별 기준 텍스트로 합성.
  // ⚠ 변경: 이전엔 mount 1회만 로드 → admin 이 기준을 바꿔도 이미 열린 탭은 시드를 계속 사용했음.
  //    이제 settings 모달 열 때 + 분석 실행 직전에 자동 refetch. 모달 안 새로고침 버튼으로 수동 갱신도 가능.
  ...
  const [criteriaLastLoadedAt, setCriteriaLastLoadedAt] = useState(0);
  // 공통 로더 — 마운트 / 설정 모달 오픈 / 분석 직전 어디서든 호출 가능.
  const refreshCriteria = useCallback(async () => {
    ...
    try {
      ...
      setCriteriaByType(next);
      setCriteriaLastLoadedAt(Date.now());
      console.log('[DesignEvaluator] criteria refreshed', Object.fromEntries(Object.entries(next).map(([k, v]) => [k, v.versionName])));
      return next;
    } catch (e) {
      console.warn("[DesignEvaluator] criteria load failed; using current state", e);
      return null;
    } finally { setCriteriaLoading(false); }
  }, []);
  useEffect(() => { refreshCriteria(); }, [refreshCriteria]);
  // 설정 모달이 열릴 때마다 최신 admin 변경사항 재로드 — 같은 세션 내 admin 편집을 즉시 반영.
  useEffect(() => { if (isSettingsOpen) refreshCriteria(); }, [isSettingsOpen, refreshCriteria]);
...
▶ [브랜드웹_모바일] 카테고리 (모바일 단일 화면 mockup — 750×~1234):
${formatCriteriaList(criteriaByType.brandwebMobile.items)}
...
- 모션 타이포 카테고리는 키프레임 한 장으로 평가하지만, 가능한 한 시간 축의 흔적(motion blur, sequential frames, easing 흔적 등)을 추론하여 timing/loop/모션 관련 항목에도 점수를 매기세요.`;
  ...
  const processFile = (file) => {
      if (file && file.type.startsWith('image/')) {
          ...
          setResultData(null);
          setManualScoreAdj(0);
          // 새 이미지 — 사용자 override 리셋. fitMode 자동 결정은 아래 effect 가 처리.
          fitModeOverrideRef.current = false;
          const img = new Image();
  ...
  const handleAnalyze = async () => {
      ...
      setManualScoreAdj(0);
      try {
          // 분석 직전 평가 기준 / rules 를 한번 더 최신화 — admin 의 변경사항이 그대로 반영되도록.
          // 응답을 기다리되 실패해도 현재 메모리 값으로 계속 진행.
          try { await refreshCriteria(); } catch { /* keep going */ }
  ...
  · "기타"
  ★ 모바일/데스크톱 구분 기준 (매우 중요):
     - 이미지 가로폭이 캔버스 짧은 변 기준 800px 이하 또는 좁은 폭(폰 베젤·단일 컬럼 레이아웃)이 명확하면 모바일.
     - 단일 폰 화면 mockup(세로 9:14~9:22 부근, 보통 750×~1234) → "브랜드웹_모바일" 또는 "프로모션 모바일".
     - 같은 모바일이라도 단일 화면 단위면 "브랜드웹_모바일", 길게 스크롤되는 랜딩이면 "프로모션 모바일".
  (타이포 구분 팁: 글자 자체에 3D 입체감/재질감이 있으면 "렌더링 타이포". 모션블러나 글자 주변에 시간성 효과(궤적)가 있으면 "모션 타이포". 평면이면 "2D 타이포".)`
...
반드시 지정된 JSON 스키마에 맞추어 10개 평가 항목 전체를 단 하나도 누락 없이 답변하세요.`;
  ...
  const ANCHOR_TYPE_TABS = [
    ...
    { id: CRITERIA_TYPES.promotionMobile, label: '프로모션 모바일' },
    { id: CRITERIA_TYPES.brandweb, label: '브랜드웹 메인' },
    { id: CRITERIA_TYPES.brandwebSub, label: '브랜드웹 서브' },
    { id: CRITERIA_TYPES.brandwebMobile, label: '브랜드웹 모바일' },
          ...
          <div className="w-full bg-white/5 border border-white/10 rounded-xl p-2 mb-4 flex gap-2 items-center shadow-lg">
              <div className="relative flex-1">
                  <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="w-full bg-transparent border-none pl-3 pr-7 py-1.5 text-[11px] font-medium text-white appearance-none focus:outline-none cursor-pointer">
                      ...
                      <option value="브랜드웹_모바일" className="bg-zinc-900">📱 브랜드웹 모바일 (750×~1234)</option>
          ...
          >
            ...
            >
                ...
                >
                    {previewUrl ? (
                        <>
                            {/* fitMode === 'screen': 화면 전체에 이미지 contain (브랜드웹 모바일 / 세로 배너 등 단일 화면).
                                fitMode === 'width': 폭 100% + 세로 스크롤 (긴 landing / 프로모션 페이지). */}
                            {fitMode === 'screen' ? (
                                <div
                            ...
                            <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2 z-10">
                                {/* fitMode 토글 — aspect 자동 결정 후에도 사용자가 override 가능. */}
                                <button
                                    onClick={() => { fitModeOverrideRef.current = true; setFitMode(m => m === 'screen' ? 'width' : 'screen'); }}
                                    className={`p-2.5 rounded-xl text-white transition-colors backdrop-blur-md border border-white/10 shadow-lg ${fitMode === 'screen' ? 'bg-[#df6a78]/80 hover:bg-[#df6a78]' : 'bg-black/70 hover:bg-white/20'}`}
                                    title={fitMode === 'screen' ? '화면 맞춤 (단일 화면) — 클릭 시 폭 맞춤으로 전환' : '폭 맞춤 (세로 스크롤) — 클릭 시 화면 맞춤으로 전환'}
                                >
                                    {fitMode === 'screen' ? <Maximize2 className="w-5 h-5" /> : <StretchVertical className="w-5 h-5" />}
                                </button>
                                ...
                                <div className="text-[10px] text-violet-300 bg-violet-500/10 border border-violet-500/30 rounded-md px-3 py-2 mb-2">
                                    <div className="flex items-start justify-between gap-2 mb-1">
                                        <div className="leading-relaxed">
                                            평가 기준은 <b>NEXUS Admin → 평가 기준 관리</b>에서 관리됩니다.
                                            {criteriaLoading
                                                ? " (불러오는 중...)"
                                                : criteriaLastLoadedAt
                                                    ? ` · 마지막 갱신 ${new Date(criteriaLastLoadedAt).toLocaleTimeString()}`
                                                    : ""}
                                        </div>
                                        <button
                                            onClick={refreshCriteria}
                                            disabled={criteriaLoading}
                                            title="admin 에서 막 바꾼 기준이 반영 안 됐을 때 사용"
                                            className="text-[10px] px-2 py-1 rounded border border-violet-400/40 bg-violet-500/10 hover:bg-violet-500/20 text-violet-200 disabled:opacity-50 shrink-0"
                                        >
                                            {criteriaLoading ? '...' : '🔄 다시 불러오기'}
                                        </button>
                                    </div>
                                    {/* 7개 type 활성 버전 표시 — "(시드)"/"(시드 fallback)" 이면 admin 에 저장되지 않았거나 로드 실패. */}
                                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[10px] text-violet-200/80">
                                        {[
                                            ['배너',            'banner'],
                                            ['프로모션',         'promotion'],
                                            ['프로모션 모바일',  'promotionMobile'],
                                            ['브랜드웹 메인',    'brandweb'],
                                            ['브랜드웹 서브',    'brandwebSub'],
                                            ['브랜드웹 모바일',  'brandwebMobile'],
                                            ['2D 타이포',       'typo2d'],
                                            ['렌더 타이포',      'typoRender'],
                                            ['모션 타이포',      'typoMotion'],
                                        ].map(([label, key]) => {
                                            const v = criteriaByType[key]?.versionName || '';
                                            const isSeed = v.includes('시드');
                                            return (
                                                <span key={key} className={isSeed ? 'text-zinc-500' : 'text-violet-100'}>
                                                    <span className="opacity-70">{label}:</span> <b>{v || '?'}</b>
                                                </span>
                                            );
                                        })}
                                    </div>
