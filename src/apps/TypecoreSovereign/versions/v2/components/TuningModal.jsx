/* eslint-disable */
// v2 전용 튜닝룸 모달 (아이디어/리터칭 룸 공용).
import React from 'react';
import { MessageSquare, X, Loader2, Image as ImageIcon, Play, CheckCircle } from 'lucide-react';
...
const TuningModal = ({
    ...
    onApply, applyLabel,
    onImageUpload, placeholder = "수정 요청이나 이미지 분석을 지시하세요.",
}) => {
        ...
        <div className="fixed inset-0 z-[2000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-[460px] h-[750px] max-h-[90vh] bg-[#121212] border border-zinc-800 rounded-[12px] shadow-2xl flex flex-col relative overflow-hidden">
                ...
                <div className="p-4 border-b border-zinc-800/50 bg-[#121212] shrink-0">
                    ...
                    <p className="text-[13px] text-emerald-300 bg-emerald-500/10 leading-relaxed max-h-[150px] overflow-y-auto custom-scrollbar whitespace-pre-wrap px-3 py-2.5 border-l-[3px] border-emerald-500 rounded-[6px]">"{String(currentValue)}"</p>
                </div>
