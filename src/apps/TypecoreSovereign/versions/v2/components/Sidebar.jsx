/* eslint-disable */
// v2 전용 메인 사이드바 — 페르소나 선택, 입력, 옵션 카드 등 좌측 패널 전체.
import React from 'react';
import {
    Command, LayoutTemplate, Anchor, Brush, Settings, Activity, Sparkles, Sparkles as SparkleIcon,
    ChevronDown, Wand, FastForward,
    Loader2, X,
    Download, FileUp, RefreshCcw,
    Image as ImageIcon, Box as BoxIcon, Highlighter,
    Bot, Layers3,
    MessageSquare, Edit3, SlidersHorizontal, Crown, AlertCircle, Pipette,
    PenTool
} from 'lucide-react';
...
import { staticOptions, sliderDesc } from '../constants/options.js';
import { getOptionName } from '../constants/utils.js';

const Sidebar = ({ rp, theme, version, setVersion, versions }) => {
    const {
        ...
        moodImageRef, isAnalyzingMood, handleMoodImageUpload,
        referenceExtractRef, isExtractingReference, handleReferenceExtraction,
        openTuningRoom, openEditTuningRoom, handleExpandIntent, isExpandingIntent,
        personaSliderValue, setPersonaSliderValue,
        editUploadedImage, setEditUploadedImage, editImageRef, handleEditImageUpload,
        ...
        handleAiRecommendation, isRecommending, setIsImportModalOpen, handleReset, aiRecSummary,
    ...
    return (
        <aside className={`${isSidebarOpen ? 'w-[380px]' : 'w-0 border-none opacity-0 m-0 p-0'} shrink-0 border border-zinc-800/80 bg-[#141414] rounded-[16px] flex flex-col shadow-2xl relative overflow-hidden transition-all duration-300 z-50`}>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
                ...
                {/* 3. AI Recommend & Reset Buttons (Creation only) */}
                {!isEditMode && (
