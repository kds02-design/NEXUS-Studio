/* eslint-disable */
// v1 전용 튜닝룸 모달 — 아이디어 튜닝룸 / 마이크로 리터칭 룸이 동일한 구조라 공용 컴포넌트로 분리.
import React from 'react';
import {
    MessageSquare, X, Edit3, Settings, Loader2, ImageIcon, Play, CheckCircle
} from 'lucide-react';

const TuningModal = ({
    isOpen, onClose, title, currentLabel, currentValue, currentLabelIcon,
    chatHistory, chatScrollRef, isLoading, loadingMessage,
    referenceImage, setReferenceImage,
    inputValue, setInputValue, onSend,
    onApply, applyLabel,
    onImageUpload,
}) => {
            ...
            <div className="w-full max-w-[460px] h-[750px] max-h-[90vh] bg-[#121212] border border-zinc-800 rounded-[10px] shadow-2xl flex flex-col relative overflow-hidden">
                ...
                <div className="p-4 border-b border-zinc-800/50 bg-[#121212] shrink-0">
                    <div className="flex items-center gap-1.5 mb-2">
                        {currentLabelIcon}
                        <span className="text-[11px] font-bold text-[#a6a6a6] tracking-wider uppercase">{currentLabel}</span>
                    </div>
                    <p className="text-[13px] text-indigo-300 bg-indigo-500/10 leading-relaxed max-h-[150px] overflow-y-auto custom-scrollbar whitespace-pre-wrap px-3 py-2.5 border-l-[3px] border-indigo-500 rounded-[6px]">"{currentValue}"</p>
                </div>
