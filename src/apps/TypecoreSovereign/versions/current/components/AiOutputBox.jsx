  ...
  <div className={`rounded-[10px] p-6 sm:p-8 border bg-[#121212] border-zinc-800 relative transition-all duration-500 mt-4`}>
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
      <div className="flex items-center gap-3 sm:gap-4 flex-wrap w-full sm:w-auto">
        <p className="text-[12px] font-bold uppercase text-[#a6a6a6] flex items-center gap-2 font-sans"><Terminal className="w-4 h-4" /> {modelState} Assembly</p>
        {outdatedFlag && (
          <span className="text-[10px] text-amber-500 font-bold flex items-center gap-1 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20 font-sans" title="옵션이 변경되었습니다. 최신 프롬프트를 위해 다시 생성해주세요.">
    ...
    <div className={`max-w-[800px] w-full mx-auto text-left whitespace-pre-wrap text-[13px] leading-relaxed p-6 rounded-[10px] border bg-[#1C1C1C] border-zinc-800 transition-colors duration-500 text-zinc-300 ${outdatedFlag ? 'opacity-60 grayscale' : 'opacity-100'}`} style={{ fontFamily: "'Noto Sans KR', sans-serif" }}>
      {content}
    </div>
  </div>
);
