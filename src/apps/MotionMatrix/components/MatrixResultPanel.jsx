...
import {
  AlertCircle, Image as ImageIcon, Upload, X, Loader2,
  Sliders, Activity, Video as VideoIcon, Download, Save, Zap, Sparkles, Minimize2, RotateCcw,
} from 'lucide-react';
import { EXPORT_MODES, EXPORT_MODE_INFO } from '../constants/presets';
import LumKey from '../../LumKey';
    ...
    <div className="flex-1 flex flex-col gap-5 overflow-hidden min-h-0 p-5">
      ...
      <div className="flex-1 flex gap-5 overflow-hidden min-h-0">
        ...
        <div className="flex-1 min-w-0 bg-[#18181B] border border-zinc-800 rounded-2xl flex flex-col overflow-hidden">
          <div className="flex justify-between items-center px-6 py-4 border-b border-zinc-800 bg-[#121214] gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold text-zinc-200 tracking-wider uppercase">Target Model Prompt</span>
              {p.isCompressed && (
                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/40 text-emerald-300 text-[9px] font-bold uppercase tracking-widest">
                  <Sparkles className="w-2.5 h-2.5" /> 압축됨
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              ...
              {p.onCompress && (
                p.isCompressed ? (
                  <button onClick={p.onClearCompressed}
                    title="압축 해제 — 원본 프롬프트로 돌리기"
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-zinc-700 text-zinc-400 hover:text-zinc-200 hover:border-zinc-500 text-[11px] font-semibold transition-colors">
                    <RotateCcw className="w-3.5 h-3.5" /> 원본
                  </button>
                ) : (
                  <button onClick={p.onCompress} disabled={p.isCompressing || !p.combinedOutput}
                    title="충돌·중복·모호 토큰을 제거하고 짧게 정리 (Analysis 크레딧 1회)"
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-emerald-500/50 text-emerald-300 hover:bg-emerald-500/10 text-[11px] font-semibold transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                    {p.isCompressing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Minimize2 className="w-3.5 h-3.5" />}
                    {p.isCompressing ? '압축 중…' : 'AI 압축'}
                  </button>
                )
              )}
...
export default function MatrixResultPanel(props) {
  const isCompositing = props.activeTab === 'compositing';
  return (
    <div className={`flex-1 h-full flex flex-col overflow-hidden ${isCompositing ? '' : 'bg-[#0f0f11] rounded-2xl border border-zinc-800'}`}>
      {isCompositing ? <LumKey accentColor="#FDCB6E" paletteVariant="muted" /> : <GenerateView {...props} />}
    </div>
  );
}
