...
export default function UpgradeRequestModal({ onClose }) {
  ...
  const btn = (bg, fg, opacity=1) => ({
    background: bg, color: fg, border:`1px solid ${bg === T.border ? T.border : bg}`,
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", backdropFilter:"blur(4px)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center", padding:20, fontFamily:"'Noto Sans KR', sans-serif" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width:"100%", maxWidth:440, background:T.surface, border:`1px solid ${T.border}`, borderRadius:14, color:T.text, overflow:"hidden" }}>
        <div style={{ padding:"22px 24px 18px", borderBottom:`1px solid ${T.border}` }}>
          <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.14em", color:T.accent, textTransform:"uppercase", marginBottom:6 }}>등급 업그레이드 요청</div>
          <div style={{ fontSize:16, fontWeight:700 }}>현재 등급: <span style={{ color:T.accent }}>{GRADE_LABEL[grade]}</span></div>
        </div>
        ...
        {done ? (
          <div style={{ padding:"32px 24px", textAlign:"center" }}>
            ...
            <div style={{ fontSize:12, color:T.textMuted, marginBottom:20 }}>관리자가 검토 후 등급을 조정합니다.</div>
            <button onClick={onClose} style={btn(T.accent, "#fff")}>닫기</button>
          </div>
        ) : (
          <>
            <div style={{ padding:"20px 24px" }}>
              ...
              <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={4} placeholder="업그레이드가 필요한 이유를 자세히 적어주세요."
                style={{ width:"100%", padding:"10px 12px", fontSize:13, background:T.bg, color:T.text, border:`1px solid ${T.border}`, borderRadius:6, outline:"none", resize:"vertical", fontFamily:"inherit", lineHeight:1.5, boxSizing:"border-box" }} />
              {err && <div style={{ fontSize:11, color:"#ff7575", marginTop:8 }}>{err}</div>}
            </div>
            <div style={{ padding:"14px 24px", background:T.bg, borderTop:`1px solid ${T.border}`, display:"flex", gap:8, justifyContent:"flex-end" }}>
              <button onClick={onClose} disabled={busy} style={btn(T.border, T.text)}>취소</button>
              <button onClick={submit} disabled={busy || !reason.trim()} style={btn(T.accent, "#fff", busy || !reason.trim() ? 0.4 : 1)}>
                {busy ? "전송 중…" : "요청 보내기"}
              </button>
            </div>
          </>
