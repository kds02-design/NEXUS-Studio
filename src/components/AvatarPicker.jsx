...
export default function AvatarPicker({ onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", backdropFilter:"blur(4px)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width:"100%", maxWidth:420, background:THEME.surface, border:`1px solid ${THEME.border}`, borderRadius:14, color:THEME.text, overflow:"hidden", fontFamily:"'Noto Sans KR', sans-serif" }}>
        <div style={{ padding:"20px 22px 14px", borderBottom:`1px solid ${THEME.border}`, display:"flex", alignItems:"center", gap:14 }}>
          <UserAvatar profile={profile} user={user} size={48} />
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:15, fontWeight:700, color:THEME.text }}>아바타 변경</div>
            <div style={{ fontSize:11, color:THEME.textMuted, marginTop:2 }}>선택 즉시 저장됩니다.</div>
          </div>
          <button onClick={onClose} style={{ background:"none", border:0, color:THEME.textMuted, fontSize:18, cursor:"pointer", padding:4 }}>✕</button>
        </div>
        ...
        <div style={{ padding:"16px 18px 20px" }}>
                ...
                return (
                  <button key={emoji} onClick={() => apply("emoji", emoji)} disabled={busy}
                    style={{
                      ...
                      background: isCurrent ? `${THEME.accent}22` : "rgba(255,255,255,0.04)",
          ...
          {tab === "initial" && (
            <>
              <div style={{ fontSize:11, color:THEME.textMuted, marginBottom:10 }}>이메일 첫 글자: <strong style={{ color:THEME.text }}>{initial}</strong></div>
                  ...
                  return (
                    <button key={c.key} onClick={() => apply("initial", c.hex)} disabled={busy}
                      style={{
                        ...
                        border:`1.5px solid ${isCurrent ? c.hex : THEME.border}`,
                      ...
                      <div style={{ fontSize:10, color:isCurrent ? c.hex : THEME.textMuted, fontWeight:600 }}>{c.label}</div>
                    </button>
                  );
                })}
              </div>
            ...
            <div>
              ...
              {current.kind === "photo" && (
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:8, marginBottom:14 }}>
                  <div style={{ width:80, height:80, borderRadius:"50%", overflow:"hidden", border:`2px solid ${THEME.border}` }}>
                    <img src={current.url} alt="현재 아바타" style={{ width:"100%", height:"100%", objectFit:"cover" }} />
                  </div>
                  <div style={{ fontSize:10, color:THEME.textDim, letterSpacing:"0.08em", textTransform:"uppercase" }}>현재 아바타</div>
                </div>
              )}
              ...
              <button
                ...
                style={{
                  ...
                  border:`1.5px dashed ${uploading ? THEME.accent : THEME.border}`,
                  borderRadius:10, transition:"all 0.12s",
                  display:"flex", flexDirection:"column", alignItems:"center", gap:10,
                  color: THEME.textMuted,
                  fontFamily:"inherit",
                }}
                onMouseEnter={(e) => { if (!uploading && !busy) e.currentTarget.style.borderColor = THEME.accent; }}
                onMouseLeave={(e) => { if (!uploading && !busy) e.currentTarget.style.borderColor = THEME.border; }}
              >
                {uploading ? (
                  <>
                    <div style={{ width:24, height:24, border:`2px solid ${THEME.border}`, borderTopColor: THEME.accent, borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />
                    <div style={{ fontSize:12, fontWeight:600, color:THEME.accent }}>업로드 중…</div>
                    <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
                  </>
                ) : (
                  <>
                    <div style={{ fontSize:28 }}>📷</div>
                    <div style={{ fontSize:13, fontWeight:600, color:THEME.text }}>이미지 선택하기</div>
                    <div style={{ fontSize:10, color:THEME.textDim }}>JPEG · PNG · WebP · 최대 5MB</div>
                  </>
                )}
              </button>
              ...
              <div style={{ fontSize:10, color:THEME.textDim, marginTop:12, lineHeight:1.6 }}>
