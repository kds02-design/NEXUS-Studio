...
export default function InviteCodeModal({ onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", backdropFilter:"blur(4px)", zIndex:10000, display:"flex", alignItems:"center", justifyContent:"center", padding:20, fontFamily:"'Noto Sans KR', sans-serif" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width:"100%", maxWidth:380, background:T.surface, border:`1px solid ${T.border}`, borderRadius:14, color:T.text, overflow:"hidden" }}>
        <div style={{ padding:"22px 24px 18px", borderBottom:`1px solid ${T.border}` }}>
          <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.14em", color:T.accent, textTransform:"uppercase", marginBottom:6 }}>초대 코드</div>
          <div style={{ fontSize:15, fontWeight:700 }}>Expert 등급 코드 입력</div>
          <div style={{ fontSize:11, color:T.textMuted, marginTop:6 }}>유효한 코드면 즉시 Expert로 업그레이드됩니다.</div>
        </div>
        ...
        {done ? (
          <div style={{ padding:"32px 24px", textAlign:"center" }}>
            ...
            <div style={{ fontSize:12, color:T.textMuted, marginBottom:20 }}>이제 무제한으로 사용할 수 있습니다.</div>
          ...
          <>
            <div style={{ padding:"20px 24px" }}>
              <input value={code} onChange={(e) => setCode(e.target.value)} onKeyDown={(e) => e.key === "Enter" && submit()}
                ...
                style={{ width:"100%", padding:"12px 14px", fontSize:14, background:T.bg, color:T.text, border:`1px solid ${T.border}`, borderRadius:6, outline:"none", fontFamily:"'JetBrains Mono', monospace", letterSpacing:"0.08em", textTransform:"uppercase", boxSizing:"border-box" }} />
            ...
            <div style={{ padding:"14px 24px", background:T.bg, borderTop:`1px solid ${T.border}`, display:"flex", gap:8, justifyContent:"flex-end" }}>
              ...
              <button onClick={submit} disabled={busy || !code.trim()} style={btn(T.accent, "#fff", busy || !code.trim() ? 0.4 : 1)}>
                {busy ? "확인 중…" : "적용"}
              </button>
            </div>
          </>
